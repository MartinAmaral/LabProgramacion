    ...
    do{
        ...
        if(cin.fail() || valor<0 || valor >1){
            ...
            cout << "Diagnósticos registrado exitosamente." << endl;
                    ...
                    // Preguntar si quiere agregar tratamiento
            int agregarTratamiento = -1;
            do {
                cout << "Ingrese 1 para agregar un tratamiento o 0 para no agregar\n";
                cin >> agregarTratamiento;
                if (cin.fail() || agregarTratamiento < 0 || agregarTratamiento > 1) {
                    cin.clear();
                    cout << "\nOpcion invalida, intentelo de nuevo.\n\n";
                    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    agregarTratamiento = -1;
                }
            } while (agregarTratamiento == -1);

            if (agregarTratamiento == 1) {
