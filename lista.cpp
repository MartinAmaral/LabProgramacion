...
listaA elim(listaA l, Archivo elem)
{
    ...
    else
    {
        ...
        while (iter != NULL)
        {
            ...
            if (_stricmp(nomb, nombreComparar) == 0)
            {
                listaA ant = iter->ant;
                listaA sig = iter->sig;
                if (l == iter)
...
bool PERTENECE_LISTA(listaA l, Cadena Nombre)
{
    ...
    {
        if (_stricmp(Nombre, DEVOLVER_NOMBRE(l->dato)) == 0)
            return true;
        else
...
listaA InsertarOrdenado(listaA l, Archivo arch)
{
    ...
    else
    {
        ...
        while (!seInserto)
        {
            ...
            if (_stricmp(nombreInsertar, nombreComparar) < 0)
            {
...
Archivo DEVOLVER_ARCHIVO(listaA lista, Cadena nombre)
{
    ...
    if (_stricmp(DEVOLVER_NOMBRE(aux->dato), nombre) == 0)
    {
        return aux->dato;
    }
    else
