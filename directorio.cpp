...
void Agregarhijo(directorio padre, directorio hijo)
{
	...
	{
		if (_stricmp(padre->pr_hijo->nombre, hijo->nombre) > 0)
		{
...
void InsertarDirOrdenado(directorio dir, directorio insertar)
{
	...
	else if (_stricmp(dir->sig_herm->nombre, insertar->nombre) > 0)
	{
...
void ImprimirRaizDir(directorio actual, Cadena nombre)
{
	...
	else if (_stricmp(actual->nombre, nombre) == 0)
	{
		std::cout << actual->nombre;
	}
	else
...
bool ExisteDirectorio(directorio dir, Cadena nombre)
{
	...
	while (hijo != NULL)
	{
		if (_stricmp(hijo->nombre, nombre) == 0)
		{
			return true;
		}
		hijo = hijo->sig_herm;
...
void ELIMINAR_DIRECTORIO(directorio dir, Cadena nombre)
{
	...
	else
	{
		while (hijo != NULL)
		{
			if (_stricmp(hijo->sig_herm->nombre, nombre) == 0)
			{
				std::cout << "entro" << '\n';
				aux = hijo->sig_herm;
...
void Moverdirectorio(directorio actual, directorio mover, directorio destino)
{
	...
	while (aux != NULL)
	{
		if (_stricmp(aux->nombre, mover->nombre) == 0)
		{
