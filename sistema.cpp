...
TipoRet CD(Sistema &s, Cadena nombreDirectorio)
{
	...
	if (_stricmp(nombreDirectorio, "RAIZ") == 0)
	{
		if (s->actual == s->raiz)
		{
	...
	else if (_stricmp(nombreDirectorio, "..") == 0)
	{
		if (_stricmp(nombreDir, "RAIZ") == 0)
		{
			std::cout << "Yo nos encontramos en la raiz del sistema";
			return ERROR;
		}
...
TipoRet MOVE(Sistema &s, Cadena nombre, Cadena directorioDestino)
{
	...
	else if (ExisteDirectorio(s->actual, nombre))
	{
		if (dirDestino != NULL)
		{
			...
			if (dirDestino == GetPadre(s->actual) && (_stricmp(nombreActual, nombre) == 0))
			{
...
TipoRet DIR(Sistema &s, Cadena parametro)
{
	...
	if (parametro != NULL && _stricmp(parametro, "/s") == 0)
	{
